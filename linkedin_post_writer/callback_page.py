"""OAuth callback page handler for LinkedIn authorization."""
...
<div class="card" id="card">
  ...
  <h1 id="title">LinkedIn Authorization</h1>
...
(function() {
  ...
  if (error) {
    ...
    subtitle.textContent = "Copy the code below and paste it into the chat to connect your LinkedIn account.";
...
function copyCode() {
  ...
  // Fallback for insecure contexts (e.g. http://<public-ip>) where
  ...
  function fallbackCopy() {
    ...
    try {
      ...
      if (ok) {
        ...
        btn.innerHTML = "Copy failed — select & ⌘C";
      }
    } catch (e) {
      btn.innerHTML = "Copy failed — select & ⌘C";
    } finally {
